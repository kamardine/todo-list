...
<template>
  <li class="py-4 flex items-center justify-between">
    <div class="flex items-center">
      ...
      />
      <svg v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="gray" class="w-5 h-6 mr-1">
        <path stroke-linecap="round" stroke-linejoin="round" d="M17.25 8.25L21 12m0 0l-3.75 3.75M21 12H3" />
      </svg>

      <p class="text-xl font-medium text-gray-900" :class="{ 'line-through text-slate-400': item.checked === true }">{{item.libelle}}</p>
    </div>
    <div class="flex">
      <button
        @click="editItem"
        class="hover:bg-none group flex items-center rounded-md bg-none px-1 py-1 mr-2 shadow-sm"
        :class="item.checked === true ? 'bg-slate-200 pointer-events-none cursor-not-allowed' : 'bg-white'">
      ...
      <button
        ...
        :class="item.checked === true || item.edited === true ? 'bg-slate-200 pointer-events-none' : 'bg-white'">
