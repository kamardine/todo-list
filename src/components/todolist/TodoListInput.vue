...
watch(() => props.editableItem, (newEditableItem, oldEditableItem) => {
  if(props.editableItem.edited === true && newEditableItem !== oldEditableItem) {
...
const btnPlaceHolder = computed(() => {
  return props.editableItem.edited === true ? 'Edit a task...' : 'Add a task...';
});
...
<template>
  <form @submit.prevent="handleSubmit" class="flex group relative">
    ...
    />
    <button type="submit" class="hover:bg-orange-500 group flex items-center rounded-md bg-orange-600 text-white text-sm font-medium pl-2 pr-3 py-2 shadow-sm">
      <svg v-if="editableItem.edited === true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="white" class="mr-1 w-4 h-4">
